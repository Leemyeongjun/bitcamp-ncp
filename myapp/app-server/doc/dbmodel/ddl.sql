...
-- 대댓글
DROP TABLE IF EXISTS hl_re_comment RESTRICT;
...
-- 게시판유형
DROP TABLE IF EXISTS hl_post_type RESTRICT;
...
CREATE TABLE hl_comment (
  ...
  created_date DATE         NOT NULL DEFAULT now() COMMENT '댓글작성일' -- 댓글작성일
)
COMMENT '댓글';
...
CREATE TABLE hl_report (
  ...
  created_date DATE         NOT NULL DEFAULT now() COMMENT '신고일', -- 신고일
  answer       VARCHAR(255) NULL     COMMENT '신고답변', -- 신고답변
  answer_date  DATE         NULL     DEFAULT now() COMMENT '답변일' -- 답변일
)
COMMENT '신고';
...
-- 대댓글
CREATE TABLE hl_re_comment (
  re_comment_id INTEGER      NOT NULL COMMENT '대댓글번호', -- 대댓글번호
  member_id     INTEGER      NOT NULL COMMENT '회원번호', -- 회원번호
  comment_id    INTEGER      NOT NULL COMMENT '댓글번호', -- 댓글번호
  content       VARCHAR(255) NOT NULL COMMENT '대댓글내용', -- 대댓글내용
  created_date  DATE         NOT NULL DEFAULT now() COMMENT '대댓글작성일' -- 대댓글작성일
)
COMMENT '대댓글';

-- 대댓글
ALTER TABLE hl_re_comment
  ADD CONSTRAINT PK_hl_re_comment -- 대댓글 기본키
  PRIMARY KEY (
  re_comment_id -- 대댓글번호
  );
...
CREATE TABLE hl_inquiry (
  ...
  created_date DATE         NOT NULL DEFAULT now() COMMENT '문의일', -- 문의일
  answer       VARCHAR(255) NULL     COMMENT '답변', -- 답변
  answer_bool  INTEGER      NOT NULL COMMENT '답변여부', -- 답변여부
  answer_date  DATE         NOT NULL DEFAULT now() COMMENT '답변일' -- 답변일
)
COMMENT '1:1문의';
...
CREATE TABLE hl_file (
  ...
  post_id      INTEGER      NOT NULL COMMENT '게시글번호', -- 게시글번호
  name         VARCHAR(50)  NOT NULL COMMENT '파일명', -- 파일명
  COL          VARCHAR(255) NULL     COMMENT '파일경로', -- 파일경로
  created_date DATE         NOT NULL DEFAULT now() COMMENT '등록일', -- 등록일
...
CREATE TABLE hl_alarm_log (
  ...
  alarm_type_id INTEGER      NOT NULL COMMENT '알림유형번호', -- 알림유형번호
  member_id     INTEGER      NOT NULL COMMENT '회원번호', -- 회원번호
  alarm_date    DATE         NOT NULL DEFAULT now() COMMENT '알림일', -- 알림일
...
-- 게시판유형
ALTER TABLE hl_post_type
  ADD CONSTRAINT PK_hl_post_type -- 게시판유형 기본키
  PRIMARY KEY (
  post_type_id -- 게시판유형번호
  );

ALTER TABLE hl_post_type
  MODIFY COLUMN post_type_id INTEGER NOT NULL AUTO_INCREMENT COMMENT '게시판유형번호';
...
-- 게시글
ALTER TABLE hl_post
  ADD CONSTRAINT FK_hl_post_type_TO_hl_post -- 게시판유형 -> 게시글
  FOREIGN KEY (
  post_type_id -- 게시판유형번호
  )
  REFERENCES hl_post_type ( -- 게시판유형
  post_type_id -- 게시판유형번호
  );
...
-- 대댓글
ALTER TABLE hl_re_comment
  ADD CONSTRAINT FK_hl_comment_TO_hl_re_comment -- 댓글 -> 대댓글
  FOREIGN KEY (
  comment_id -- 댓글번호
  )
...
-- 대댓글
ALTER TABLE hl_re_comment
  ADD CONSTRAINT FK_hl_member_TO_hl_re_comment -- 회원 -> 대댓글
  FOREIGN KEY (
  member_id -- 회원번호
  )
  REFERENCES hl_member ( -- 회원
